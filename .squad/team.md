# Squad Team

> exceptionless-rs — Rust client for Exceptionless

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. Does not generate domain artifacts. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Leela | Lead | `.squad/agents/leela/charter.md` | ✅ Active |
| Bender | Rust SDK Dev | `.squad/agents/bender/charter.md` | ✅ Active |
| Farnsworth | Integration Dev | `.squad/agents/farnsworth/charter.md` | ✅ Active |
| Amy | Tester | `.squad/agents/amy/charter.md` | ✅ Active |
| Fry | Docs/DevRel | `.squad/agents/fry/charter.md` | ✅ Active |
| Scribe | Session Logger | `.squad/agents/scribe/charter.md` | 📋 Silent |
| Ralph | Work Monitor | — | 🔄 Monitor |

## Project Context

- **Owner:** Christian Helle
- **Project:** exceptionless-rs
- **Stack:** Rust, Cargo, Exceptionless API integration, GitHub Actions
- **Description:** An idiomatic Rust client for Exceptionless, porting key behaviors from Exceptionless.Net for errors, logs, and feature usage.
- **Created:** 2026-05-18T10:43:35.499+02:00
